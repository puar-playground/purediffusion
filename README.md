...
ddpm = DDPMPipline(num_timesteps=1000, device='cpu', beta_schedule='cosine')
```
...
ddpm.ddpm_reverse(model, batch_size, data_shape)
```
...
```
ddim = DDIMPipline(ddpm, ddim_num_steps=100)
```
To generate data:
```
ddim.ddim_reverse(model, batch_size, data_shape)
```
